...
class DencoderBlock(nn.Module):
    ...
    #     return x
    def forward(self , x  , encoder_output, src_masck , tgt_mask):
        x=self.connection[0](x, lambda x : self.self_attentio(x,x,x,tgt_mask))
        # result from the encoder  the k and the value 
        x=self.connection[1](x, lambda x : self.self_attentio(x,encoder_output,encoder_output,src_masck))
        x=self.connection[2](x, self.feed_forword)
        return x
...
class Dencoder(nn.Module):
    ...
    def forward(self , x  ,encoder_output, src_masck , tgt_mask):
        for layer in self.layers:
            x=layer(x  , encoder_output, src_masck , tgt_mask)
        return self.norm(x)
